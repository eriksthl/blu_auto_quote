/* BLU Auto Quote v10.2.2
 * Web automation target adapter. Shared quotation changes are in v10.2.2-features.js.
 * Pricing is taken from the existing engine, not recalculated here.
 * Web-only collection labels and input behaviour are applied separately.
 */
(() => {
  "use strict";
  ...
  const legacy = Object.freeze({
    ...
    restoreDirectionStateV912, createChargeMaps, lclCreateCharges
  });
  ...
  "timing": {
    ...
    "rowDelay": 800,
    "tabDelay": 600,
    "lookupExtraDelay": 1000,
    "routeLookupExtraDelay": 2500,
    "cargoDetailsTabDelay": 2000
  },
  "transportModes": {
    "t": "truck",
    "truck": "truck",
  ...
  "suppliers": {
    "msc sweden": "msc sweden ab",
    "hapag-lloyd": "hapag-lloyd",
    "cma cgm": "cma cgm",
    "maersk sverige": "maersk sverige ab",
    "oocl": "oocl (sweden) ab",
    "one1": "one1",
    "cosco": "cosco",
    "overseas liner": "overseas liner agencies ab",
    "greencarr consol": "greencarr consol",
    "dania": "Dania",
    "skara": "skara",
    "pgf": "pgf",
    "imex lager": "imex lagerservice ab",
    "thyr": "thyr harbour sören",
    "hcc": "hcc",
    "chs": "chs",
    "sr group": "sr group as",
    "n.a.": "n.a. nissen",
    "nordicon sverige": "nordicon sverige",
    "shipco transport": "shipco transport ab"
  },
  "incoterms": {
    "exw": "Ex works",
    "fca": "Free carrier"
  ...
  const validSaveCoordinate = (value,limit) => Number.isInteger(Number(value)) && Number(value)>0 && Number(value)<limit ? Number(value) : 0;
  const defaultSavePosition = () => ({x:CONFIG.coordinates["save button"][0],y:CONFIG.coordinates["save button"][1]});
  function validSavePosition(x,y) {
    x=validSaveCoordinate(x,1920);y=validSaveCoordinate(y,1080);
    return x && y ? {x,y} : null;
  }
  let webSavePosition = defaultSavePosition(), webSaveOverride = false;
  try {
    const saved=JSON.parse(localStorage.getItem("blu.autoquote.web-save-position") || "null") || {};
    const position=validSavePosition(saved.x,saved.y);
    if(position) { webSavePosition=position;webSaveOverride=true; }
  } catch {}
  function setSavePosition(x,y) {
    const position=validSavePosition(x,y);
    webSavePosition = position || defaultSavePosition();
    webSaveOverride = Boolean(position);
    try {
      if(position) localStorage.setItem("blu.autoquote.web-save-position",JSON.stringify(position));
      else localStorage.removeItem("blu.autoquote.web-save-position");
    } catch {}
    if($("bluSaveX")) $("bluSaveX").value=webSavePosition.x;
    if($("bluSaveY")) $("bluSaveY").value=webSavePosition.y;
    invalidateGeneratedScripts();
  }
  ...
  function offsetFor(kind, index) {
    ...
    // The updated charge pane has fixed screen coordinates for every charge.
    if (kind === "charge") return CONFIG.chargeOffsets[Math.min(index, CONFIG.chargeOffsets.length) - 1];
    if (kind === "chargeField") return CONFIG.chargeFieldOffsets[Math.min(index, CONFIG.chargeFieldOffsets.length) - 1];
  ...
  // Web lookup rules only; keep every Citrix supplier string/arrow untouched.
  function supplierSelection(value, arrowOffset = 0) {
    const supplier = String(value ?? "").trim();
    const key = supplier.toLowerCase();
    return Object.prototype.hasOwnProperty.call(CONFIG.suppliers, key)
      ? { search: CONFIG.suppliers[key], arrowOffset: 0 }
      : { search: supplier, arrowOffset };
  }

  // Use the same display name as the subject, but retain the actual UN/LOCODE
  // and all pricing, quantities, suppliers and manually named unrelated rows.
  function webCollectionChargeNames(charges, canonical, displayName, lcl = false) {
    const original = normalisedLocationName(canonical || "");
    const display = normalisedLocationName(displayName || "");
    if (!original || !display || original === display) return charges;
    return charges.map(charge => {
      const type = String(charge.type || "").trim().toLowerCase();
      const description = String(charge.description || "");
      const eligible = type === "precarriage" || (lcl && type === "trucking");
      if (!eligible || !description.includes(original)) return charge;
      return { ...charge, description: description.replace(original, () => display) };
    });
  }
  createChargeMaps = function(route, equipment, calc) {
    const charges = legacy.createChargeMaps(route, equipment, calc);
    return automationMode === "web"
      ? webCollectionChargeNames(charges, routeCollection(route), route?.collectionDisplayName)
      : charges;
  };
  lclCreateCharges = function(calc) {
    const charges = legacy.lclCreateCharges(calc);
    return automationMode === "web" && !isImportMode()
      ? webCollectionChargeNames(charges, lclCollectionValue(), state.lcl?.collectionDisplayName, true)
      : charges;
  };
  ...
  exportFormData = function() {
    ...
    return {...data,appVersion:VERSION,automationMode};
  };
  ...
  function extractQuoteBlock(source) {
    ...
    if (start < 0) throw new Error("The quotation engine did not return quotation data.");
  ...
  function webSaveAhk(merged = false) {
    return String.raw`CaptureWebSavePosition() {
  global Running, WEB_SAVE_X, WEB_SAVE_Y, WEB_SAVE_OVERRIDE, BLU_WINDOW
  if Running
    return
  if !WinActive(BLU_WINDOW) {
    MsgBox "Activate BLU Web, move the mouse over Save, then press F7.", "BLU Web Save", "Icon!"
    return
  }
  MouseGetPos &x, &y
  if x <= 0 || x >= 1920 || y <= 0 || y >= 1080
    return
  WEB_SAVE_X := x, WEB_SAVE_Y := y
  WEB_SAVE_OVERRIDE := true
  IniWrite x, A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "X"
  IniWrite y, A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "Y"
  MsgBox "Save position recorded: " x ", " y ${merged ? '". Press F8 to start the batch."' : '". Press F10 to save."'}, "BLU Web Save", "Iconi"
}

RequireWebSavePosition() {
  global WEB_SAVE_X, WEB_SAVE_Y, WEB_SAVE_OVERRIDE
  ; Explicit app coordinates win; otherwise keep an existing F7 calibration.
  ; Missing/invalid calibration leaves the measured default pair intact.
  if !WEB_SAVE_OVERRIDE {
    try {
      savedX := Number(IniRead(A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "X", "0"))
      savedY := Number(IniRead(A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "Y", "0"))
      if savedX > 0 && savedX < 1920 && savedY > 0 && savedY < 1080 {
        WEB_SAVE_X := savedX, WEB_SAVE_Y := savedY
      }
    } catch {
      ; Keep the measured defaults when a stored calibration cannot be read.
    }
    WEB_SAVE_OVERRIDE := true
  }
  if WEB_SAVE_X <= 0 || WEB_SAVE_X >= 1920 || WEB_SAVE_Y <= 0 || WEB_SAVE_Y >= 1080
    throw Error("Save position is missing. Activate BLU Web, hover over Save and press F7, then press ${merged ? 'F8' : 'F10'}.")
}

SaveWebQuote() {
  global WEB_SAVE_X, WEB_SAVE_Y
  RequireWebSavePosition()
${merged ? '  ActivateBluV923()\n  Send "{Esc}"\n  Loop 6 {\n    Send "{WheelUp 20}"\n    Sleep 60\n  }\n  Sleep 600' : '  CheckWebEnvironment()\n  ActivateBluWeb()\n  WebScroll("up")'}
  Click WEB_SAVE_X, WEB_SAVE_Y
${merged ? "  Sleep 3000" : "  WebWait(1200)"}
}
`;
  }
  function buildWebScript(source, lcl) {
    const quoteBlock = extractQuoteBlock(source).replace(/("equipmentSearch",\s*")20['’]\s*dv"/gi, (_match, prefix) => prefix + "20' dv\"");
    ...
    if ($("bluWebGenerationNote")) {
      $("bluWebGenerationNote").textContent = manualBuying
        ? "Buying-only rows require manual entry in BLU Web before saving."
        : "";
...
; Generated by BLU Auto Quote v10.2.2 - BLU Web Mode
; BLU_AUTOMATION_MODE: WEB
; Quotation engine v10.2.2. Web pickup labels use custom collection names.
; Start with the BLU Web tab active in Edge, on the recorded 1920x1080 / 100% layout.
; F8 starts; F6 continues after Create; F9 pauses/resumes; Esc aborts.
; F7 captures Save; F10 clicks Save. No sending.
; Detail step: 11.4 px, rounded. Equipment 5 is interpolated at Y=269.
; Add charge is fixed at 1007,616; charge fields are fixed from the Y=638 reference.
; No route/detail/charge-row offsets apply to the fixed charge pane.
; Scroll to bottom ONCE per detail; do not re-anchor between individual charges.
; Details Y correction remains until scroll up; the fixed charge pane is excluded.
; Reference: transit Y=313 after one removal; subsequent removals use the row step.
...
LOOKUP_EXTRA_DELAY := ${CONFIG.timing.lookupExtraDelay}
ROUTE_LOOKUP_EXTRA_DELAY := ${CONFIG.timing.routeLookupExtraDelay}
CARGO_DETAILS_TAB_DELAY := ${CONFIG.timing.cargoDetailsTabDelay}
...
WEB_VALID_TO := ${ahkString(data.validTo || "")}
DETAIL_STEP := ${CONFIG.detailStep}
DETAILS_REFERENCE_DY := ${CONFIG.detailsReference.transitTimeY - CONFIG.coordinates["transit time"][1]}
DETAILS_REFERENCE_REMOVED := ${CONFIG.detailsReference.removedRouteRows}
WEB_DETAILS_DY := 0
EQUIPMENT_OFFSETS := [${CONFIG.equipmentOffsets.join(", ")}]
CHARGE_OFFSETS := [${CONFIG.chargeOffsets.join(", ")}]
CHARGE_FIELD_OFFSETS := [${CONFIG.chargeFieldOffsets.join(", ")}]
WEB_FIXED_CHARGE_FIELDS := Map(${CONFIG.fixedChargeFields.map(name=>ahkString(name)+", true").join(", ")})
...
WEB_SUPPLIERS := ${ahkDictionary(CONFIG.suppliers)}
Running := false
WEB_WAITING_AFTER_CREATE := false
SavedClipboard := ""
WebHwnd := 0
WEB_SAVE_X := ${Number(webSavePosition.x) || 0}
WEB_SAVE_Y := ${Number(webSavePosition.y) || 0}
WEB_SAVE_OVERRIDE := ${webSaveOverride ? "true" : "false"}
...
F6::ContinueWebAfterCreate()
F9::Pause -1
F7::CaptureWebSavePosition()
F10:: {
  global Running
  if Running
    return
  try SaveWebQuote()
    ...
    MsgBox err.Message, "BLU Web Save", "Icon!"
}
$Esc::ExitApp 1

RunQuote() {
  global quote, WEB_LCL, WEB_IMPORT, WEB_TARGET_CURRENCY, WEB_VALID_TO, SavedClipboard, ROW_DELAY, TAB_DELAY, CARGO_DETAILS_TAB_DELAY, WEB_DETAILS_DY
  CheckWebEnvironment()
  ActivateBluWeb()
  ...
  WaitForWebCreateContinue()

  ToolTip "BLU Web: header", 20, 30
  WebScroll("up")
  ; Create quotation already opens Header. Do not click its tab again.
  ; HTML date inputs/export use YYYY-MM-DD; do not reuse the Citrix DD.MM.YY value.
  WebTypeField("valid to date", WEB_VALID_TO, 0, true)
  ...
  WebScroll("up")
  WebClick("cargo tab", 0, CARGO_DETAILS_TAB_DELAY)
  FillWebCargo()

  ToolTip "BLU Web: details and charges", 20, 30
  WebScroll("up")
  WebClick("details tab", 0, CARGO_DETAILS_TAB_DELAY)
  ...
  for index, detail in details {
    ...
    removedRows := ConfigureWebRoute(detail, dy)

    ; The measured reference already includes one removal. Carry the shared
    ; correction through Details; the fixed charge pane is excluded by WebClick.
    WEB_DETAILS_DY := WebDetailsOffset(removedRows)
    WebScroll("down")
    ...
    remarksOffset := chargeIndex > 0 ? WebOffset("chargeField", chargeIndex) : 0
    WebPasteField("charges remarks", detail.Get("remarks", ""), remarksOffset)
  }
  ToolTip
  WebBuyingOnlyNotice()
  RestoreClipboard()
  elapsedSeconds := Round((A_TickCount - startedAt) / 1000)
  duration := Floor(elapsedSeconds / 60) " min " Mod(elapsedSeconds, 60) " sec"
  MsgBox "BLU Web entry completed." Chr(10) "Time: " duration Chr(10) Chr(10) "Review all fields and manual buying rows. Press F10 to click Save. F7 records the Save button position if it has not been set.", "BLU Web Mode", "Iconi"
}

WaitForWebCreateContinue() {
  global WEB_WAITING_AFTER_CREATE
  ; An earlier/held F6 must not skip this quotation's checkpoint.
  KeyWait "F6"
  WEB_WAITING_AFTER_CREATE := true
  ToolTip "Quotation created. Review BLU, then press F6 to continue.", 20, 30
  try {
    while WEB_WAITING_AFTER_CREATE
      Sleep 50
  } finally {
    WEB_WAITING_AFTER_CREATE := false
    ToolTip
...
ContinueWebAfterCreate() {
  global WEB_WAITING_AFTER_CREATE
  if WEB_WAITING_AFTER_CREATE
    WEB_WAITING_AFTER_CREATE := false
}
...
WebOffset(kind, index) {
  global DETAIL_STEP, EQUIPMENT_OFFSETS, CHARGE_OFFSETS, CHARGE_FIELD_OFFSETS
  if index < 1
    throw Error("Invalid Web row index.")
  if kind = "charge"
    return CHARGE_OFFSETS[Min(index, CHARGE_OFFSETS.Length)]
  if kind = "chargeField"
    return CHARGE_FIELD_OFFSETS[Min(index, CHARGE_FIELD_OFFSETS.Length)]
  if kind = "equipment" {
...
WebClick(name, dy := 0, wait := 0) {
  ...
  point := C[name]
  x := point[1], y := point[2] + WebCoordinateOffset(name, dy)
...
WebCoordinateOffset(name, dy) {
  global WEB_FIXED_CHARGE_FIELDS, WEB_DETAILS_DY
  if WEB_FIXED_CHARGE_FIELDS.Has(name)
    return 0
  return dy + WEB_DETAILS_DY
}
...
WebReady(name) {
  global LOOKUP_EXTRA_DELAY, ROUTE_LOOKUP_EXTRA_DELAY
  key := StrLower(name)
  ; Preserve client +1000 ms; route searches get +2500 ms on top before Enter.
  ; Keep the existing menu wait and all other field-specific waits unchanged.
  isRouteCode := key = "plor loc code" || key = "polout loc code" || key = "podin loc code" || key = "plod loc code"
  if key = "client fhb" || isRouteCode
    WebWait(LOOKUP_EXTRA_DELAY)
  if isRouteCode
    WebWait(ROUTE_LOOKUP_EXTRA_DELAY)
...
WebScroll(direction) {
  global TAB_DELAY, WEB_DETAILS_DY
  EnsureWebActive()
  Send "{Esc}"
...
WebDetailsOffset(removedRows) {
  global DETAIL_STEP, DETAILS_REFERENCE_DY, DETAILS_REFERENCE_REMOVED
  ; Keep the working correction for route-dependent Details fields.
  ; WebCoordinateOffset excludes the updated fixed charge pane.
  return DETAILS_REFERENCE_DY + Round(DETAILS_REFERENCE_REMOVED * DETAIL_STEP) - Round(removedRows * DETAIL_STEP)
}
...
ConfigureWebRoute(detail, dy) {
  global MENU_ENTER_DELAY, DETAIL_STEP
  ...
  ; Remaining selectors stay put; the Delete button moves UP after each removal.
  deleted := 0
  Loop 4 {
    index := 5 - A_Index
    if !enabled[index] {
      WebClick(selectors[index], dy)
      WebClick("delete selected event", dy - Round(deleted * DETAIL_STEP), MENU_ENTER_DELAY)
      deleted += 1
    }
  }
  return deleted
...
WebSelectSupplier(value, dy := 0, arrowOffset := 0) {
  global WEB_SUPPLIERS
  supplier := Trim(String(value))
  key := StrLower(supplier)
  if WEB_SUPPLIERS.Has(key) {
    supplier := WEB_SUPPLIERS[key]
    arrowOffset := 0
  }
  WebSelect("supplier", supplier, dy, arrowOffset)
}
...
FillWebCharge(charge, index) {
  ...
  addDy := WebOffset("charge", index)
  dy := WebOffset("chargeField", index)
  WebClick("add sales charge 1", addDy, ROW_DELAY)
  ...
  per := WEB_LCL ? StrLower(Trim(String(charge.Get("per", "")))) : "q"
  ; LCL flat charges use Web's default: no per/quantity/consignment input.
  ; Keep volume, weight and explicit W/M bases; FCL still sets quantity + unit.
  skipQuantityBasis := WEB_LCL && (per = "" || per = "q" || per = "quantity" || per = "consignment")
  if !skipQuantityBasis
    WebSelect("sales rate per (quantity/volume/weight etc.)", per, dy)
  ; The engine's weight rates are per tonne. Select the Web unit explicitly;
  ; BLU converts the Cargo weight. Do not overwrite its auto-populated amount.
  if per = "w"
    WebSelect("weight unit", "ton", dy)
  ; Volume keeps its automatic amount/unit; explicit W/M remains separate.
  if !skipQuantityBasis && per != "v" && per != "w" {
    if per = "weight_m" {
  ...
  if charge["supplier"] != ""
    WebSelectSupplier(charge["supplier"], dy, charge.Get("supplierArrow", 0))
}
...
`;
  ...
  ToolTip
  message := "Web quote " index " has been entered. Review it, complete any manual buying-only rows, and return to the top of the page in BLU Web. Click OK here to click Save and continue. Cancel stops the remaining batch."
  if MsgBox(message, "BLU Web - review and save", "OKCancel Default2") != "OK"
    throw Error("Batch stopped by user after Web quote " index ".")
  SaveWebQuote()
}`);
    source += "\n" + webSaveAhk(true);
    const firstSource=String(items[0]?.content || "");
    const childPosition=validSavePosition(firstSource.match(/^WEB_SAVE_X := (\d+)$/m)?.[1],firstSource.match(/^WEB_SAVE_Y := (\d+)$/m)?.[1]);
    const childFlag=firstSource.match(/^WEB_SAVE_OVERRIDE := (true|false)$/m)?.[1];
    // Older scripts have no flag: a valid non-zero pair was explicitly set.
    const childOverride=Boolean(childPosition) && childFlag !== "false";
    const savePosition=webSaveOverride ? webSavePosition : childOverride ? childPosition : defaultSavePosition();
    const saveOverride=webSaveOverride || childOverride;
    source = source.replace(/^Running :=/m, `WEB_SAVE_X := ${savePosition.x}\nWEB_SAVE_Y := ${savePosition.y}\nWEB_SAVE_OVERRIDE := ${saveOverride ? "true" : "false"}\nRunning :=`)
      .replace(/^F8::/m, "F7::CaptureWebSavePosition()\n\nF8::")
      .replace('  MergedTempDir := A_Temp', '  RequireWebSavePosition()\n  MergedTempDir := A_Temp');
  ...
  const collectionInputTitles = new WeakMap();
  function syncModeUi() {
    ...
    if ($("bluWebSavePosition")) $("bluWebSavePosition").hidden = !web;
    document.querySelectorAll('input[data-v964-location-key="collection"]').forEach(input => {
      if (!collectionInputTitles.has(input)) collectionInputTitles.set(input, input.title);
      input.title = web
        ? "Changes subject, subtitle and pickup charge name. The UN/LOCODE logic remains unchanged."
        : collectionInputTitles.get(input);
    });
  ...
  function installModeUi() {
    ...
    style.textContent = `
      ...
      .blu-web-note[hidden]{display:none!important;}
    ...
    const save=document.createElement("div");save.id="bluWebSavePosition";
    save.style.cssText="display:flex;gap:8px;align-items:center;flex-wrap:wrap;flex-basis:100%";
    save.innerHTML='<label>Save X <input id="bluSaveX" type="number" min="1" max="1919" placeholder="X" style="width:75px"></label><label>Y <input id="bluSaveY" type="number" min="1" max="1079" placeholder="Y" style="width:75px"></label><span>Or hover over Save in BLU Web and press F7 in the generated script. F10 saves.</span>';
    bar.append(save);
    $("bluSaveX").value=webSavePosition.x || '';$("bluSaveY").value=webSavePosition.y || '';
    for(const id of ["bluSaveX","bluSaveY"]) $(id).addEventListener("change",()=>setSavePosition($("bluSaveX").value,$("bluSaveY").value));
    const styleSave=document.createElement('style');styleSave.textContent='#bluWebSavePosition[hidden]{display:none!important}';document.head.append(styleSave);
    syncModeUi();
  }
  installModeUi();
  window.BLU_AUTO_QUOTE_V1022 = window.BLU_AUTO_QUOTE_V1021 = window.BLU_AUTO_QUOTE_V1020 = window.BLU_AUTO_QUOTE_V1013 = window.BLU_AUTO_QUOTE_V1012 = window.BLU_AUTO_QUOTE_V1011 = window.BLU_AUTO_QUOTE_V1010 = window.BLU_AUTO_QUOTE_V1000 = Object.freeze({
    version: VERSION, mode: () => automationMode, setMode: setAutomationMode,
    config: CONFIG, offsetFor, chargeSearch, incotermSearch, transportModeSearch, supplierSelection, webCollectionChargeNames, extractQuoteBlock,
    legacyFcl: legacy.generateAhk, legacyLcl: legacy.generateLclAhk,
    buildWebScript, setSavePosition
  });
})();
