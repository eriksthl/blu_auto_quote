/* BLU Auto Quote v10.3.2
 * Web automation target adapter. Shared quotation changes are in v10.3.2-features.js.
...
`;
  ...
  function buildWebScript(source, lcl, options = {}) {
    ...
    const grossExpression = bulk ? ahkString(data.grossWeight || '') : (source.match(/TypeField\("gross weight", ("(?:`.|[^"\n])*?")\)/)?.[1] || ahkString(FIXED.grossWeight));
    const cargoExpression = bulk ? ahkString(data.descriptionOfGoods || '') : (source.match(/TypeField\("description of goods", ("(?:`.|[^"\n])*?")/)?.[1] || ahkString(FIXED.goodsDescription));
...
; Generated by BLU Auto Quote v10.3.2 - BLU Web Mode
; BLU_AUTOMATION_MODE: WEB
; Quotation engine v10.3.2. Web pickup labels use custom collection names.
...
RunQuote() {
  global quote, WEB_LCL, WEB_IMPORT, WEB_TARGET_CURRENCY, WEB_VALID_TO, SavedClipboard, ROW_DELAY, TAB_DELAY, CARGO_DETAILS_TAB_DELAY
  CheckWebEnvironment()
  ActivateBluWeb()
  ...
  ${bulk ? 'WebTypeField' : 'WebSelect'}("client fhb", quote["clientCode"])
  ...
  for index, detail in details {
    ...
    WebPasteField("short description", ${bulk ? 'StrUpper(quote["clientName"])' : 'StrUpper(quote["clientName"]) Chr(13) Chr(10) detail["basisText"]'}, dy)
    ConfigureWebRoute(detail, dy)

    ; Full scroll down aligns the bottom fields to fixed screen positions.
    ; Removed route rows affect Delete only, never these bottom coordinates.
    WebScroll("down")
    WebTypeField("transit time", detail.Get("transitTime", quote.Get("transitTime", "")))
${bulk ? `    if detail["incoterm"] != "" {
      WebSelect("incoterm", WebIncoterm(detail["incoterm"]))
      WebTypeField("incoterm location", detail["incotermLocation"])
    } else {
      WebTypeField("incoterm", "")
      WebTypeField("incoterm location", "")
    }` : `    WebSelect("incoterm", WebIncoterm(detail["incoterm"]))
...
WebCoordinateOffset(name, dy) {
  global WEB_FIXED_CHARGE_FIELDS
...
WebReady(name) {
  global LOOKUP_EXTRA_DELAY
  key := StrLower(name)
  ; Route lookups have their own paced entry and final one-second wait.
  if key = "client fhb"
...
WebSelect(name, value, dy := 0, arrowOffset := 0) {
  ...
  if WebIsRouteCode(name) {
    WebRouteCode(name, value, dy)
    return
  }
  WebClick(name, dy)
  WebWait(200)
...
WebRouteCode(name, value, dy := 0) {
  global MENU_AFTER_DELAY
  ; Dedicated focus wait, then four half-second gaps for a five-letter code.
  ; Ctrl+A lets the first literal character replace any previous selection.
  WebClick(name, dy, 500)
  EnsureWebActive()
  Send "^a"
...
`;
  ...
  installModeUi();
  window.BLU_AUTO_QUOTE_V1030 = window.BLU_AUTO_QUOTE_V1024 = window.BLU_AUTO_QUOTE_V1023 = window.BLU_AUTO_QUOTE_V1022 = window.BLU_AUTO_QUOTE_V1021 = window.BLU_AUTO_QUOTE_V1020 = window.BLU_AUTO_QUOTE_V1013 = window.BLU_AUTO_QUOTE_V1012 = window.BLU_AUTO_QUOTE_V1011 = window.BLU_AUTO_QUOTE_V1010 = window.BLU_AUTO_QUOTE_V1000 = Object.freeze({
