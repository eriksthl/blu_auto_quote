/* Shared quotation changes in v10.2.0. Loaded after the existing engine. */
(() => {
  'use strict';
  ...
  const versionData = data => ({...data,appVersion:'10.2.0'});
  ...
  quoteText = function(key,...args) {
    if (key==='common.grossWeight' && String(state.fclGrossWeight || '').trim()) return String(state.fclGrossWeight);
    return base.quoteText(key,...args);
  };
  function setWeight(source) {
    state.fclGrossWeight=String(source?.fclGrossWeight ?? base.quoteText('common.grossWeight') ?? '25000');
    if ($('fclGrossWeightV1020')) $('fclGrossWeightV1020').value=state.fclGrossWeight;
  }
  exportFormData = function() { return {...versionData(base.exportFormData()),fclGrossWeight:String(state.fclGrossWeight || '25000')}; };
  ...
  function versionScript(source) { return source.replace(/^(; Generated by BLU Auto Quote[^\n]*?)v[\d.]+/m,'$1v10.2.0'); }
  ...
  $('fclGrossWeightV1020')?.addEventListener('input',event=>{state.fclGrossWeight=event.target.value;});
