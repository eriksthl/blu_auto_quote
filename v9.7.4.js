(() => {
  ...
  function v972InstallUi() {
    ...
    // v9.7.4: supplier PDFs are loaded from the contextual CHS/Shipco controls.
  ...
  function v972ApplyChsLcl(parsed) {
    ...
    data.oceanCarrier = String(parsed.carrierText || "").trim();
...
(() => {
  ...
  function v973PatchLclAhk(script) {
    let output = String(script || "").replaceAll("v9.7.4", `v${V973_VERSION}`).replaceAll("v9.6.4", `v${V973_VERSION}`);
...
`    if charge["per"] = "weight_m" {
  ...
  generateAhk = function() {
    return String(v973Base.generateAhk()).replaceAll("v9.7.4", `v${V973_VERSION}`).replaceAll("v9.6.4", `v${V973_VERSION}`);
  };
...
// v9.7.4: requested quotation, calculation and automation fixes.
(() => {
  "use strict";
  const VERSION = "9.7.4";
  const base = {
    newRoute, newLclState, exportFormData, exportLclData, applyImportedData, applyImportedLclData,
    augmentFriendly: augmentFriendlyBluQuoteV964, loadRoute: loadActiveRouteIntoForm,
    setAdvanced: setAdvancedModeV920, activateMode, resetForm, resetLclForm,
    routePathParts, lclSubject, lclBasis: lclBasisTextV960, lclRemarksText,
    calculateLcl, lclCreateCharges, updateLclCalculation, generateAhk, generateLclAhk,
    validateLcl
  };
  ...
  const plain = (value, digits = 4) => Number.isFinite(Number(value)) ? String(Number(Number(value).toFixed(digits))) : "";
  const escape = (value) => escapeHtmlV912(String(value ?? ""));
  const copy = (value) => JSON.parse(JSON.stringify(value));
  let automation = { slowMode: false, speedPercent: 100 };

  function normaliseAutomation(value = {}) {
    const raw = Number(value.speedPercent);
    return { slowMode: Boolean(value.slowMode), speedPercent: Number.isFinite(raw) ? Math.max(1, Math.min(100, Math.round(raw))) : 100 };
  }
  function speedPercent() { return automation.slowMode ? automation.speedPercent : 100; }
  function extraCities(source) {
    return (Array.isArray(source) ? source : []).map((city) => ({
      name: text(typeof city === "string" ? city : city?.displayName || city?.name || "")
    }));
  }
  function collectionNames(primary, extras) {
    const names = [primary, ...extraCities(extras).map((city) => city.name)]
      .map((name) => normalisedLocationName(name)).filter(Boolean);
    return [...new Set(names)].join("/");
  }
  function normaliseTerms(source, fallback = {}) {
    const result = { ...source };
    const mode = normaliseSearch(result.freeDaysMode || "");
    if (["specify", "specified", "custom"].includes(mode)) result.freeDaysMode = "custom";
    else if (!mode) {
      const supplied = ["freeDays", "freeDaysDemurrage", "freeDaysDetention"].some((key) => own(result, key) && text(result[key]) !== "");
      result.freeDaysMode = supplied ? "custom" : fallback.freeDaysMode || "standard";
    }
    return result;
  }
  function prepareRoute(source = {}) {
    const result = normaliseTerms(source);
    result.additionalCollections = extraCities(source.additionalCollections);
    if (Array.isArray(source.equipment)) result.equipment = source.equipment.map((equipment) => {
      const normalised = normaliseTerms(equipment, result);
      // Carry route-level terms through all older newEquipment migrations.
      V950_DETAIL_TERM_KEYS.forEach((key) => {
        if (!own(normalised, key) && own(result, key)) normalised[key] = result[key];
      });
      return normalised;
    });
    return result;
  ...
  newRoute = function(overrides = {}) {
    const source = prepareRoute(overrides);
    const route = base.newRoute(source);
    route.additionalCollections = extraCities(source.additionalCollections);
    return route;
  };
  newLclState = function(overrides = {}) {
    const result = base.newLclState(overrides);
    result.additionalCollections = extraCities(overrides.additionalCollections);
    result.oceanCarrier = text(overrides.oceanCarrier);
    return result;
  };
  exportFormData = function() {
    const result = base.exportFormData();
    result.appVersion = VERSION;
    result.automation = { ...automation };
    result.routes = (result.routes || []).map((route, index) => ({ ...route,
      additionalCollections: extraCities(state.routes?.[index]?.additionalCollections || route.additionalCollections)
    }));
    return result;
  };
  exportLclData = function() {
    return { ...base.exportLclData(), appVersion: VERSION, automation: { ...automation },
      additionalCollections: extraCities(state.lcl?.additionalCollections), oceanCarrier: text(state.lcl?.oceanCarrier)
    };
  };
  applyImportedData = function(data) {
    const prepared = { ...data, routes: Array.isArray(data?.routes) ? data.routes.map(prepareRoute) : data?.routes };
    if (data?.automation) automation = normaliseAutomation(data.automation);
    const result = base.applyImportedData(prepared);
    syncUi();
    return result;
  };
  applyImportedLclData = function(data) {
    if (data?.automation) automation = normaliseAutomation(data.automation);
    const result = base.applyImportedLclData(data);
    state.lcl.additionalCollections = extraCities(data?.additionalCollections);
    state.lcl.oceanCarrier = text(data?.oceanCarrier);
    syncUi();
    updateLclCalculation();
    return result;
  };
  augmentFriendlyBluQuoteV964 = function(data, result) {
    const augmented = base.augmentFriendly(data, result);
    if (!augmented?.metadata) return augmented;
    const settings = data?.settings || {};
    if (settings.automation || data.automation) augmented.metadata.automation = normaliseAutomation(settings.automation || data.automation);
    if (augmented.mode === "FCL") {
      augmented.metadata.routes = (augmented.metadata.routes || []).map((route, index) => prepareRoute({
        ...route, additionalCollections: data.routes?.[index]?.additionalCollections || route.additionalCollections
      }));
    } else {
      augmented.metadata.additionalCollections = extraCities(data.route?.additionalCollections || data.routes?.[0]?.additionalCollections);
      augmented.metadata.oceanCarrier = text(data.oceanCarrier || data.route?.oceanCarrier || settings.oceanCarrier);
    }
    return augmented;
  };

  routePathParts = function(route) {
    const parts = base.routePathParts(route);
    if (!route || isImportMode() || !routePartsV961(route).collection) return parts;
    return parts.map((part, index) => index === 0 ? collectionNames(part, route.additionalCollections) : part);
  };
  lclSubject = function() {
    const subject = base.lclSubject();
    if (isImportMode() || lclIsFob() || !state.lcl?.additionalCollections?.length) return subject;
    const context = locationContextV964("lcl", "collection");
    const primary = normalisedLocationName(context?.target?.[context.field] || context?.canonical || lclCollectionValue());
    return primary ? subject.replace(primary, collectionNames(primary, state.lcl.additionalCollections)) : subject;
  };

  function lclCarrier() {
    const supplier = lclSupplierConfig();
    if (lclQuoteProfileV960() !== "CHS") return text(supplier.display);
    const value = text(state.lcl?.oceanCarrier);
    return /^(?:CHS|CHS AIR\s*(?:&|AND)?\s*SEA)$/i.test(value) ? "" : value;
  }
  lclBasisTextV960 = function(supplier, incoterm) {
    const carrier = lclCarrier();
    if (!carrier) return lclFreightCollectV960() ? `(${upper(incoterm)}, FREIGHT COLLECT)` : quoteText("common.detailIncotermOnly", { INCOTERM: incoterm });
    return base.lclBasis({ ...supplier, display: carrier }, incoterm);
  };
  lclRemarksText = function() {
    const original = base.lclRemarksText();
    if (lclQuoteProfileV960() !== "CHS") return original;
    const agentLine = quoteText("lcl.remarks.carrier", { SUPPLIER: upper(lclSupplierConfig().display) });
    const carrier = lclCarrier();
    const carrierLine = carrier ? quoteText("lcl.remarks.carrier", { SUPPLIER: upper(carrier) }) : "";
    return String(original).replace(agentLine, carrierLine).replace(/^\s+|\s+$/g, "");
  };
  validateLcl = function() {
    const result = base.validateLcl();
    if (lclQuoteProfileV960() === "CHS" && !lclCarrier()) result.warnings = [...(result.warnings || []), "Ocean carrier is missing. No agent name will be printed; enter the carrier from the supplier quotation."];
    return result;
  };

  // Use the final generated charge rows as the single calculation/display source.
  function multiplier(charge, calc) {
    if (charge.per === "weight_m") return Number(charge.amount);
    if (charge.per === "v") return calc.volume;
    if (charge.per === "w") return calc.weightTon;
    return 1;
  }
  function customerChargeDescription(value) {
    return String(value || "").replace(/\bOTHER CHS ORIGIN CHARGES\b/gi, "OTHER ORIGIN CHARGES");
  }
  function chargeKey(charge, calc) {
    const type = normaliseSearch(charge.type);
    const standard = { trucking: "trucking", freight: "ocean", delivery: "delivery", fuel: "fuel", eu: "eu", war: "war",
      terminal: "terminal", port: "port", "custom export": "customExport", "forwarding fee": "forwarding", handling: "handling", isps: "isps", profit: "profit" };
    if (standard[type]) return standard[type];
    const description = normaliseSearch(customerChargeDescription(charge.description));
    if (description === normaliseSearch(quoteText("lcl.charges.solasFee"))) return "solas";
    if (description === normaliseSearch(quoteText("lcl.charges.customsCheckMrn"))) return "customs";
    const extra = (calc.extraChargeRows || []).find((row) => description === normaliseSearch(customerChargeDescription(row.charge.outputDescription ?? row.charge.name)));
    return extra ? `extra:${extra.index}` : "";
  }
  function chargeProfitUsd(charges, calc) {
    return charges.reduce((sum, charge) => {
      const buy = text(charge.buyRate) ? lclConvert(charge.buyRate, charge.buyCurrency, "USD") : 0;
      const sell = text(charge.sellingRate) ? lclConvert(charge.sellingRate, charge.sellingCurrency || "USD", "USD") * multiplier(charge, calc) : 0;
      return sum + sell - buy;
    }, 0);
  }
  function finalCharges(calc) {
    const charges = base.lclCreateCharges(calc);
    if (calc.profile === "CHS") charges.forEach((charge) => { charge.description = customerChargeDescription(charge.description); });
    if (calc.profile !== "CHS" || !state.lcl?.v973ChsMeta) return charges;
    const overrides = state.lcl.sellingOverrides || {};
    // The previous CHS postprocessor overwrote manual terminal/documentation
    // and ocean rates. Restore explicit overrides before balancing automatic rows.
    charges.forEach((charge) => {
      const key = chargeKey(charge, calc);
      if (key && own(overrides, key) && Number.isFinite(Number(overrides[key])))
        charge.sellingRate = plain(lclConvert(overrides[key], "USD", charge.sellingCurrency || "USD"));
    });
    const inland = charges.filter((charge) => ["trucking", "delivery"].includes(chargeKey(charge, calc)));
    const ocean = charges.find((charge) => chargeKey(charge, calc) === "ocean");
    const oceanFraction = lclProfitOceanFractionV961(inland.length > 0, Boolean(calc.freightCollect), Boolean(ocean));
    const entries = charges.map((charge) => {
      const key = chargeKey(charge, calc);
      const weight = key === "ocean" ? oceanFraction : ["trucking", "delivery"].includes(key) ? (1 - oceanFraction) / Math.max(1, inland.length) : 0;
      return { charge, key, weight };
    }).filter((entry) => entry.weight > 0 && !own(overrides, entry.key));
    const totalWeight = entries.reduce((sum, entry) => sum + entry.weight, 0);
    const difference = calc.desiredProfit - chargeProfitUsd(charges, calc);
    if (totalWeight > 0 && Number.isFinite(difference)) entries.forEach(({ charge, weight }) => {
      const currency = charge.sellingCurrency || "USD";
      const addition = lclConvert(difference * weight / totalWeight / multiplier(charge, calc), "USD", currency);
      charge.sellingRate = plain(Number(charge.sellingRate) + addition);
    });
    calc.unallocatedProfit = totalWeight > 0 ? 0 : difference;
    const terminal = charges.find((charge) => chargeKey(charge, calc) === "terminal");
    if (terminal?.per === "weight_m") terminal.description = `TERMINAL HANDLING CHARGE, ${plain(terminal.sellingRate)} ${terminal.sellingCurrency}/100kg // 1cbm = ${plain(state.lcl.v973ChsMeta.ratio, 3)}kg`;
    return charges;
  }
  calculateLcl = function() {
    const calc = base.calculateLcl();
    if (!calc || isImportMode()) return calc;
    const charges = finalCharges(calc);
    const oldRows = new Map((calc.displayRows || []).map((row) => [row.key, row]));
    calc.v974Charges = charges.map((charge) => ({ ...charge }));
    calc.displayRows = charges.map((charge, index) => {
      const key = chargeKey(charge, calc);
      const previous = oldRows.get(key);
      const buying = text(charge.buyRate) ? lclConvert(charge.buyRate, charge.buyCurrency, "USD") : 0;
      const rate = lclConvert(charge.sellingRate, charge.sellingCurrency || "USD", "USD");
      const row = lclDisplayRowV942(previous?.label || charge.description || charge.type, key || `generated:${index}`, buying,
        rate, multiplier(charge, calc), Boolean(key && own(state.lcl.sellingOverrides, key)), Boolean(key));
      row.charge = { ...charge };
      if (key === "terminal" && charge.per === "weight_m") row.label = `Terminal handling (${plain(charge.sellingRate)} ${charge.sellingCurrency}/100 kg)`;
      return row;
    });
    calc.actualProfit = chargeProfitUsd(charges, calc);
    calc.negativeSelling = calc.displayRows.some((row) => row.sellingRate < 0);
    // Retain USD as the internal model currency; the table explicitly converts it.
    const fields = { ocean: "oceanSelling", trucking: "truckingSelling", delivery: "deliverySelling", terminal: "terminalSelling" };
    calc.displayRows.forEach((row) => { if (fields[row.key]) calc[fields[row.key]] = row.sellingRate; });
    calc.terminalMultiplier = calc.displayRows.find((row) => row.key === "terminal")?.multiplier;
    return calc;
  };
  lclCreateCharges = function(calc) {
    return calc?.v974Charges ? calc.v974Charges.map((charge) => ({ ...charge })) : base.lclCreateCharges(calc);
  };
  updateLclCalculation = function() {
    const result = base.updateLclCalculation();
    if (isImportMode()) return result;
    const calc = calculateLcl();
    const box = $("lclCalculation");
    if (!calc || !box) return result;
    const currency = window.BLU_AUTO_QUOTE_V973.sellingCurrency();
    const money = (usd) => formatMoney(lclConvert(usd, "USD", currency), currency, 2);
    const rows = calc.displayRows.map((row) => {
      const rate = plain(row.charge.sellingRate);
      const editor = row.editable ? `<div class="selling-rate-editor ${row.manual ? "manual" : "auto"}"><input class="lcl-selling-override-input" type="number" min="0" step="0.0001" data-lcl-selling-key="${escape(row.key)}" data-selling-currency="${escape(currency)}" value="${escape(rate)}" aria-label="${escape(row.label)} selling rate ${escape(currency)}"><button type="button" class="btn small lcl-selling-rate-reset" data-lcl-selling-key="${escape(row.key)}" ${row.manual ? "" : "disabled"} title="Return to automatic calculation">Auto</button></div>` : escape(rate);
      return `<tr><td>${escape(row.label)}</td><td>${money(row.buying)}</td><td>${editor}</td><td>${escape(plain(row.multiplier, 3))}</td><td>${money(row.totalSelling)}</td><td>${money(row.profit)}</td></tr>`;
    }).join("");
    const warning = Number.isFinite(calc.unallocatedProfit) && Math.abs(calc.unallocatedProfit) > 0.01
      ? `<div class="calculation-warning">${money(calc.unallocatedProfit)} of the requested profit cannot be redistributed. Set an Ocean freight, Trucking or Delivery rate to Auto.</div>` : "";
    box.innerHTML = `<table class="editable-calculation-table lcl-editable-calculation"><thead><tr><th>Charge</th><th>Buying total ${escape(currency)}</th><th>Selling / unit ${escape(currency)}</th><th>Qty</th><th>Selling total ${escape(currency)}</th><th>Profit total ${escape(currency)}</th></tr></thead><tbody>${rows}</tbody></table><div class="profit-line"><span>Calculated profit</span><span>${money(calc.actualProfit)} <small>(${formatMoney(lclConvert(calc.actualProfit, "USD", "SEK"), "SEK", 0)})</small></span></div>${warning}`;
    return result;
  };

  function renderCollections(scope) {
    const input = $(scope === "fcl" ? "collectionCity" : "lclCollectionCity");
    const parent = input?.closest(".field");
    if (!parent) return;
    let host = $(scope + "AdditionalCollectionsV974");
    if (!host) { host = document.createElement("div"); host.id = scope + "AdditionalCollectionsV974"; host.className = "v974-additional-collections"; parent.append(host); }
    const target = scope === "fcl" ? activeRoute() : state.lcl;
    if (!target) return;
    target.additionalCollections = extraCities(target.additionalCollections);
    host.replaceChildren();
    const note = document.createElement("div"); note.className = "small-note";
    note.textContent = "Additional cities appear in subject/subtitle. The first city remains the transport route's collection point.";
    host.append(note);
    target.additionalCollections.forEach((city, index) => {
      const row = document.createElement("div"); row.className = "v974-collection-row";
      const field = document.createElement("input"); field.type = "text"; field.value = city.name;
      field.placeholder = "Additional collection city"; field.setAttribute("aria-label", `Additional collection city ${index + 1}`);
      field.addEventListener("input", () => { target.additionalCollections[index].name = field.value; scope === "fcl" ? updateAllPreviews() : updateLclCalculation(); });
      const remove = document.createElement("button"); remove.type = "button"; remove.className = "btn small"; remove.textContent = "Remove";
      remove.addEventListener("click", (event) => { event.preventDefault(); target.additionalCollections.splice(index, 1); renderCollections(scope); scope === "fcl" ? updateAllPreviews() : updateLclCalculation(); });
      row.append(field, remove); host.append(row);
    });
    const add = document.createElement("button"); add.type = "button"; add.className = "btn small"; add.textContent = "+ Collection city";
    add.addEventListener("click", (event) => { event.preventDefault(); target.additionalCollections.push({ name: "" }); renderCollections(scope); host.querySelector(".v974-collection-row:last-of-type input")?.focus(); });
    host.append(add);
  }
  function installUi() {
    const style = document.createElement("style"); style.id = "v974Styles";
    style.textContent = `body:not(.advanced-mode-enabled) .v974-advanced, body:not(.advanced-mode-enabled) .v974-additional-collections {display:none!important;}
      .v974-speed {display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:8px;border:1px solid #bbb;margin:8px 0;}
      .v974-speed label {display:flex;align-items:center;gap:6px;white-space:nowrap;} .v974-speed input[type=checkbox] {width:16px;height:16px;flex:0 0 auto;margin:0;} .v974-speed input[type=number] {width:76px;}
      .v974-additional-collections {margin-top:8px;display:grid;gap:6px;} .v974-collection-row {display:flex;gap:6px;align-items:center;}
      .v974-collection-row input {min-width:0;flex:1;} .v974-collection-row button {flex:0 0 auto;}
      .v974-carrier-field {margin-top:8px;} .v974-speed .small-note {flex-basis:100%;}`;
    document.head.append(style);
    const advanced = $("advancedModeControl");
    if (advanced && !$("slowModeControlsV974")) {
      const panel = document.createElement("div"); panel.className = "v974-speed v974-advanced"; panel.id = "slowModeControlsV974";
      panel.innerHTML = `<label><input type="checkbox" id="slowModeV974">Slow mode</label><label>Speed <input type="number" id="speedPercentV974" min="1" max="100" step="1" value="100"> %</label><span class="small-note">100% = standard. Lower values slow typing and confirmations, not mouse movement.</span>`;
      advanced.after(panel);
      $("slowModeV974").addEventListener("change", () => { automation.slowMode = $("slowModeV974").checked; syncSpeedUi(); });
      $("speedPercentV974").addEventListener("change", () => { automation = normaliseAutomation({ ...automation, speedPercent: $("speedPercentV974").value || 100 }); syncSpeedUi(); });
      $("speedPercentV974").addEventListener("input", () => { if ($("speedPercentV974").value !== "") automation = normaliseAutomation({ ...automation, speedPercent: $("speedPercentV974").value }); });
    }
    const supplier = $("lclSupplier");
    if (supplier && !$("lclOceanCarrierV974")) {
      const field = document.createElement("label"); field.id = "lclOceanCarrierFieldV974"; field.className = "field v974-carrier-field";
      field.append(document.createTextNode("Ocean carrier (shown in quote)"));
      const input = document.createElement("input"); input.type = "text"; input.id = "lclOceanCarrierV974"; input.placeholder = "Carrier from the CHS quotation";
      input.addEventListener("input", () => { state.lcl.oceanCarrier = text(input.value); updateLclCalculation(); });
      field.append(input); supplier.closest(".field")?.after(field);
    }
    document.addEventListener("change", (event) => {
      if (["advancedMode", "lclSupplier"].includes(event.target?.id)) queueMicrotask(syncUi);
    });
    syncUi();
  }
  function syncSpeedUi() {
    if ($("slowModeV974")) $("slowModeV974").checked = automation.slowMode;
    if ($("speedPercentV974")) { $("speedPercentV974").value = automation.speedPercent; $("speedPercentV974").disabled = !automation.slowMode; }
  }
  function syncUi() {
    syncSpeedUi(); renderCollections("fcl"); renderCollections("lcl");
    const field = $("lclOceanCarrierFieldV974");
    field?.classList.toggle("hidden", lclQuoteProfileV960() !== "CHS");
    if ($("lclOceanCarrierV974")) $("lclOceanCarrierV974").value = text(state.lcl?.oceanCarrier);
  }
  loadActiveRouteIntoForm = function() { const result = base.loadRoute(); syncUi(); return result; };
  setAdvancedModeV920 = function(enabled) { const result = base.setAdvanced(enabled); syncUi(); return result; };
  activateMode = function(mode) { const result = base.activateMode(mode); syncUi(); return result; };
  resetForm = function(...args) {
    const before = state.routes; const result = base.resetForm(...args);
    if (state.routes !== before) { automation = normaliseAutomation(); syncUi(); }
    return result;
  };
  resetLclForm = function(...args) {
    const before = state.lcl; const result = base.resetLclForm(...args);
    if (state.lcl !== before) { automation = normaliseAutomation(); syncUi(); }
    return result;
  };

  // The AHK transformation is intentionally shared by all four quote workflows.
  function replaceFunction(script, name, replacement, required = true) {
    const pattern = new RegExp(`^${name}\\([^\\n]*\\) \\{[\\s\\S]*?^\\}`, "m");
    if (!pattern.test(script)) {
      if (required) throw new Error(`v9.7.4: generated AHK helper ${name} was not found.`);
      return script;
    }
    return script.replace(pattern, () => replacement);
  }
  function physicalRouteAhk() {
  ...
  function lclRouteContext() {
    const imported = isImportMode();
    const data = exportLclData();
    return {
      routeCollectionCode: imported ? "" : text(data.collectionCode || $("lclCollectionCode")?.value),
      routePolCode: text(data.polCode || state.lcl.polCode || $("lclPolCode")?.value || "SEGOT"),
      routePodCode: text(data.podCode || $("lclPodCode")?.value || "SEGOT"),
      routeDeliveryCode: text(data.deliveryCode || state.lcl.deliveryCode || ""),
      hasCollection: !imported && !lclIsFob(), hasPol: !imported || Boolean(state.lcl.includeOceanFreight), hasPod: true,
      hasDelivery: imported || Boolean(lclDeliverySelectedV963()), deliveryMode: state.lcl.deliveryMode === "RAIL" ? "r" : "t"
    };
  }
  function patchRoute(script, lcl) {
    let output = script;
    if (!output.includes('"plod mot",')) output = output.replace('  "polout/podin mot", [790, 504],', '  "polout/podin mot", [790, 504],\n  "plod mot", [790, 542],\n  "podin mot", [790, 522],');
    if (!lcl) return replaceFunction(output, "ConfigurePhysicalRoute", physicalRouteAhk());
    const context = lclRouteContext();
    const start = output.indexOf("quote := Map("); const end = output.indexOf("\n\nOnExit", start);
    if (start < 0 || end < 0) throw new Error("v9.7.4: LCL quote map was not found.");
    let map = output.slice(start, end);
    Object.keys(context).forEach((key) => { map = map.replace(new RegExp(`^  "${key}", [^\\n]+\\n`, "gm"), ""); });
    const fields = Object.entries(context).map(([key, value]) => `  "${key}", ${typeof value === "boolean" ? boolAhk(value) : ahkString(value)},`).join("\n");
    map = map.replace(/^  "charges", \[/m, `${fields}\n  "charges", [`);
    // Only customer-facing carrier labels are changed, never buying suppliers.
    const basis = isImportMode() && !state.lcl.includeOceanFreight ? quoteText("common.detailIncotermOnly", { INCOTERM: lclIncoterm() }) : lclBasisTextV960(lclSupplierConfig(), lclIncoterm());
    map = map.replace(/^  "basisText", [^\n]+/m, `  "basisText", ${ahkString(basis)},`);
    map = map.replace(/^  "supplierDisplay", [^\n]+/m, `  "supplierDisplay", ${ahkString(lclCarrier())},`);
    output = output.slice(0, start) + map + output.slice(end);
    if (/ConfigureImportRoute\(detail\) \{/.test(output)) {
      output = output.replace("ConfigureImportRoute(quote)", "ConfigurePhysicalRoute(quote)");
      output = replaceFunction(output, "ConfigureImportRoute", physicalRouteAhk());
    } else {
      const routeBlock = /  if !quote\["removePlor"\] && quote\["collectionCode"\] != ""[\s\S]*?(?=  TypeField\("transit time")/;
      if (!routeBlock.test(output)) throw new Error("v9.7.4: LCL transport route block was not found.");
      output = output.replace(routeBlock, "  ConfigurePhysicalRoute(quote)\n");
      output = output.replace("\nFormatElapsed(milliseconds) {", `\n${physicalRouteAhk()}\n\nFormatElapsed(milliseconds) {`);
    }
    return output;
  }

  function automationHelpersAhk() {
    return `WaitV974(milliseconds) {
...
TypeTextV974(value, forceTyping := false) {
  ...
  function patchAutomation(script, lcl) {
    let output = patchRoute(String(script), lcl);
    output = output.replace("SetTitleMatchMode 2", `SPEED_PERCENT := ${speedPercent()}\n\n; Optional tray icon when the ICO is next to the generated script.\nif FileExist(A_ScriptDir "\\blu_prod_20260831114214.ico")\n  try TraySetIcon(A_ScriptDir "\\blu_prod_20260831114214.ico")\n\nSetTitleMatchMode 2`);
    output = output.replace("SetKeyDelay 25, 20", "SetKeyDelay Round(25 * 100 / SPEED_PERCENT), Round(20 * 100 / SPEED_PERCENT)");
    output = replaceFunction(output, "TypeField", `TypeField(name, value, pressEnter := false, clearFirst := false) {
  ...
  if pressEnter {
    ...
    output = replaceFunction(output, "PasteCurrent", `PasteCurrent(value, clearFirst := false) {
  ...
  SendEvent "^v"
  Sleep FIELD_DELAY + 450
}`);
    output = replaceFunction(output, "PasteField", `PasteField(name, value, clearFirst := false) {
  ClickField(name, 200)
  PasteCurrent(value, clearFirst)
}`);
    output = replaceFunction(output, "PasteRemarksField", `PasteRemarksField(value) {
  global FIELD_DELAY
  ...
  Sleep 200
    ...
    // Including/excluding are always typed, including Unicode and multiline text.
    output = output.replace(/PasteField\("(including|excluding)",/g, 'TypeField("$1",');
    // PasteField's third parameter meant clearFirst, not pressEnter.
    output = output.replace(/TypeField\("(including|excluding)", ((?:detail|quote)\["(?:including|excluding)"\]), true\)/g, 'TypeField("$1", $2, false, true)');
    for (const name of ["SelectField", "SelectSupplierField", "SelectSupplierAt", "DoubleSelectField"]) {
      const pattern = new RegExp(`^${name}\\(([^\\n]*)\\) \\{[\\s\\S]*?^\\}`, "m");
      const match = output.match(pattern);
      if (!match) {
        if (name === "SelectSupplierField" || name === "SelectSupplierAt") continue;
        throw new Error(`v9.7.4: ${name} is missing.`);
      }
      const field = name === "SelectSupplierField" && !/^name\s*,/.test(match[1]) ? '"supplier"' : "name";
      const body = match[0].replace(/^(\s*)Sleep (MENU_ENTER_DELAY|SUPPLIER_ENTER_DELAY)$/m, `$1FieldReadyDelayV974(${field})\n$1Sleep $2`);
      output = output.replace(pattern, () => body);
    }
    output = output.replace(/ClickField\("Display costs"\)[\s\S]*?Send "\{Enter\}"/, (block) => block.replace('Send "{Enter}"', 'FieldReadyDelayV974("Display costs")\n  Send "{Enter}"'));
    // Any remaining direct text entry must honour slow mode as well.
    output = output.replace(/^(\s*)SendText (.+)$/gm, (line, indent, expression) => expression.trim() === "character" ? line : `${indent}TypeTextV974(${expression})`);
    // Scale all explicit waits, while keeping mouse movement/drag speed unchanged.
    output = output.replace(/^(\s*)Sleep ([^\n]+)$/gm, "$1WaitV974($2)");
    output = output.replace("\nFormatElapsed(milliseconds) {", `\n${automationHelpersAhk()}\n\nFormatElapsed(milliseconds) {`);
    return output;
  }
  generateAhk = function() { return patchAutomation(base.generateAhk(), false); };
  generateLclAhk = function() { return patchAutomation(base.generateLclAhk(), true); };

  installUi();
  updateAllPreviews();
  updateLclCalculation();
  window.BLU_AUTO_QUOTE_V974 = Object.freeze({ version: VERSION, speedPercent, syncUi,
    automation: () => ({ ...automation }), physicalRouteAhk, lclCarrier });
})();
