...
(() => {
  ...
  function buildWebScript(source, lcl) {
    ...
    const packageExpression = source.match(/SelectField\("kind of equipment", ("(?:`.|[^"\n])*?")\)/)?.[1] || '"pe"';
...
; Generated by BLU Auto Quote v10.0.0 - BLU Web Mode
...
; Charge offsets: 0,11,24,35,46,57; charge 7+ stays at 57.
...
SCROLL_ANCHOR := [${CONFIG.scrollAnchor.join(", ")}]
...
RunQuote() {
  ...
  WebScroll("up")
  WebClick("header tab", 0, TAB_DELAY)
...
WebTypeText(value, menu := false) {
  ...
  value := StrReplace(String(value), Chr(13), "")
  for character in StrSplit(value) {
    EnsureWebActive()
    if character = Chr(10)
      Send "{Enter}"
    else if character = Chr(9)
      SendText " "
    else
      SendText character
    WebWait(menu ? MENU_KEY_DELAY : 25)
  }
}
...
WebDisplayCosts() {
  ...
  WebClick("display costs")
  ; Preserve the established two-C selection sequence.
  WebTypeText("c", true)
  WebWait(180)
  WebTypeText("c", true)
  WebReady("display costs")
  EnsureWebActive()
  Send "{Enter}"
  WebWait(MENU_AFTER_DELAY)
}
...
WebScroll(direction) {
  global SCROLL_ANCHOR, TAB_DELAY
  EnsureWebActive()
  Send "{Esc}"
  ; Wheel scrolling avoids Ctrl+Home/End moving the caret in a text field.
  ; No click is made at this inferred whitespace anchor.
  MouseMove SCROLL_ANCHOR[1], SCROLL_ANCHOR[2], 0
...
ConfigureWebRoute(detail, dy) {
  ...
  if detail["hasPol"] && detail["hasPod"]
    WebDoubleSelect("polout mot", "v", dy)
  else if detail["hasCollection"] && detail["hasPod"]
    WebDoubleSelect("podin mot", "v", dy)
  if detail["hasCollection"] && detail["hasPol"]
    WebDoubleSelect("plor mot", "t", dy)
  if detail["hasDelivery"]
    WebDoubleSelect("plod mot", detail["deliveryMode"], dy)
...
FillWebCharge(charge, index) {
  ...
  if per = "v" {
    ...
    quantity := WebNumber(quote["weight"]) / 1000
    quantityName := charge.Get("weightUnit", "t")
    if quantityName = ""
      quantityName := "t"
  } else if per = "weight_m" {
  ...
  ; Weight unit changes can recalculate the numeric amount: choose unit first.
  if per = "w"
    WebSelect("sales rate amount name (40'HC, CONSIGNMENT etc)", quantityName, dy)
  else
...
`;
  ...
  installModeUi();
  window.BLU_AUTO_QUOTE_V1000 = Object.freeze({
    version: VERSION, mode: () => automationMode, setMode: setAutomationMode,
    config: CONFIG, offsetFor, chargeSearch, incotermSearch, extractQuoteBlock,
