/* BLU Auto Quote v10.2.0
 * Web automation target adapter. Shared quotation changes are in v10.2.0-features.js.
...
(() => {
  ...
  let webSavePosition = {x:0,y:0};
  try {
    const saved=JSON.parse(localStorage.getItem("blu.autoquote.web-save-position") || "null") || {};
    webSavePosition={x:validSaveCoordinate(saved.x,1920),y:validSaveCoordinate(saved.y,1080)};
  } catch {}
  function setSavePosition(x,y) {
    webSavePosition = {x:validSaveCoordinate(x,1920),y:validSaveCoordinate(y,1080)};
    try { localStorage.setItem("blu.autoquote.web-save-position",JSON.stringify(webSavePosition)); } catch {}
  ...
  function offsetFor(kind, index) {
    ...
    // The Add button keeps its sixth step; the fields stop moving at step five.
  ...
  global Running, WEB_SAVE_X, WEB_SAVE_Y, BLU_WINDOW
  if Running
    return
...
RequireWebSavePosition() {
  ...
  if !WEB_SAVE_X || !WEB_SAVE_Y {
    WEB_SAVE_X := Number(IniRead(A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "X", "0"))
    WEB_SAVE_Y := Number(IniRead(A_Temp "\BLU_Auto_Quote_Web.ini", "Save", "Y", "0"))
  }
...
; Generated by BLU Auto Quote v10.2.0 - BLU Web Mode
; BLU_AUTOMATION_MODE: WEB
; Quotation engine v10.2.0. Web pickup labels use custom collection names.
; Start with the BLU Web tab active in Edge, on the recorded 1920x1080 / 100% layout.
; F8 starts; F9 pauses/resumes; Esc aborts. F7 captures Save; F10 clicks Save. No sending.
; Detail step: 11.4 px, rounded. Equipment 5 is interpolated at Y=269.
; Add-charge offsets: 0,11,24,35,46,57; charge 6+ uses +57 px.
; Charge-field offsets: 0,11,24,35,46; charge 5+ and final remarks use +46 px.
...
`;
  ...
  SaveWebQuote()
    ...
    const saveX=Number(webSavePosition.x) || Number(firstSource.match(/^WEB_SAVE_X := (\d+)$/m)?.[1]) || 0;
    const saveY=Number(webSavePosition.y) || Number(firstSource.match(/^WEB_SAVE_Y := (\d+)$/m)?.[1]) || 0;
    source = source.replace(/^Running :=/m, `WEB_SAVE_X := ${saveX}\nWEB_SAVE_Y := ${saveY}\nRunning :=`)
  ...
  installModeUi();
  window.BLU_AUTO_QUOTE_V1020 = window.BLU_AUTO_QUOTE_V1013 = window.BLU_AUTO_QUOTE_V1012 = window.BLU_AUTO_QUOTE_V1011 = window.BLU_AUTO_QUOTE_V1010 = window.BLU_AUTO_QUOTE_V1000 = Object.freeze({
