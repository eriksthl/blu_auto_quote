/* Shared quotation changes in v10.2.2. Loaded after the existing engine. */
(() => {
  'use strict';
  ...
  const versionData = data => ({...data,appVersion:'10.2.2'});
  ...
  function versionScript(source) {
    ...
    return source.replace(fieldHeader,header=>header+'  if StrLower(name) = "gross weight"\n    value := StrReplace(String(value), ".", ",")\n');
  }
